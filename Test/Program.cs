...
string json=JsonSerializer.Serialize(allProducts);
Console.WriteLine(json);
File.WriteAllText("products.json",json);

string readjson=File.ReadAllText("product.json");
List<Product> restoreProducts=JsonSerializer.Deserialize<List<Product>>(readjson);
foreach(Product p in restoreProducts)
{
    System.Console.WriteLine( p.ProductId+" "+p.Title);
}